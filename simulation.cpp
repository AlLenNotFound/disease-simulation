...
SIR rk4Step(double S, double I, double R, double neighborInfluence) {
    auto fS = [&](double s, double i) { return -BETA * s * (i + neighborInfluence); };
    auto fI = [&](double s, double i) { return BETA * s * (i + neighborInfluence) - GAMMA * i; };
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        allStates = loadStateData("covid_data.csv");
